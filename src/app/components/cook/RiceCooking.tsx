...
export default function RiceCooking() {
    ...
    const handleDrop = useCallback((info: any) => {
        // Use document.elementsFromPoint for robust hit detection on mobile/desktop
        const elementsAtPoint = document.elementsFromPoint(info.point.x, info.point.y);
        const isOverPot = elementsAtPoint.some(el =>
            el.classList.contains("pot-target") ||
            (el instanceof HTMLElement && el.closest(".pot-target"))
        );
    ...
    return (
        <div
            className="relative min-h-[100dvh] w-full flex flex-col items-center justify-start py-8 md:py-24 p-4 md:p-12 overflow-x-hidden bg-background"
        >
            {/* Background Ambience */}
            ...
            <div className="relative z-10 w-full max-w-7xl flex flex-col items-center gap-8 md:gap-24">
                ...
                {/* Mobile: Reordered Grid (Grains closer to Pot) */}
                <div className="flex flex-col-reverse lg:flex-row items-center justify-center gap-12 lg:gap-32 w-full">
                        ...
                        <AnimatePresence mode="wait">
                            {!riceDropped ? (
                                <motion.div
                                    ...
                                    className="expert-card p-6 md:p-8 rounded-[40px] border-accent/20 flex flex-col items-center gap-4 md:gap-6 w-full sm:w-64"
                                >
                                    <div className="text-[9px] font-black text-accent uppercase tracking-[0.5em] leading-none">Sacred Grains</div>
                                    <motion.div
                                        drag
                                        dragSnapToOrigin
                                        ...
                                        className="w-24 h-24 md:w-32 md:h-32 rounded-[32px] expert-card border-accent/40 flex items-center justify-center cursor-grab active:cursor-grabbing shadow-2xl relative overflow-hidden"
                                    >
                        ...
                        >
                            ...
                            <AnimatePresence mode="wait">
                                <motion.div
                                    ...
                                    initial={{ opacity: 0, scale: 0.8 }}
                                    animate={{ opacity: 1, scale: 1 }}
                                    className="relative w-40 h-40 md:w-80 md:h-80 rounded-full expert-card flex items-center justify-center overflow-hidden"
                                >
